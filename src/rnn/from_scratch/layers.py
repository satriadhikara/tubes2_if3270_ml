...
class RNNCell:
    def __init__(self, weights_kernel, weights_recurrent, bias):
        """
        Initialize simple RNN cell with weights from Keras model
        
        Parameters:
        - weights_kernel: Input-to-hidden weights (input_dim, hidden_dim)
        - weights_recurrent: Hidden-to-hidden weights (hidden_dim, hidden_dim)
        ...
        # Determine hidden dimension (RNN state size)
    ...
    def forward(self, x, h_prev):
        ...
        """
        # Simple RNN computation: h_t = tanh(W_x * x_t + W_h * h_{t-1} + b)
...
class SimpleRNN:
    def __init__(self, weights_kernel, weights_recurrent, bias, return_sequences=False):
        """
        Initialize SimpleRNN layer with weights from Keras model
        ...
        """
        self.cell = RNNCell(weights_kernel, weights_recurrent, bias)
...
# Keep LSTM classes for backward compatibility (in case you need them)
class LSTMCell:
    def __init__(self, weights_kernel, weights_recurrent, bias):
        """
...
class Bidirectional:
    def __init__(self, forward_rnn, backward_rnn):
        """
        Initialize Bidirectional wrapper with forward and backward RNN layers
        
        Parameters:
        - forward_rnn: RNN layer for forward pass (can be SimpleRNN or LSTM)
        - backward_rnn: RNN layer for backward pass (can be SimpleRNN or LSTM)
        """
        self.forward_layer = forward_rnn  # Changed to forward_layer for consistency with parameter counting
        self.backward_layer = backward_rnn  # Changed to backward_layer for consistency with parameter counting
    ...
    def forward(self, x):
        ...
        Returns:
        - output: Concatenated output from forward and backward RNNs
        """
        # Forward direction
