    ...
    def summary(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # LSTM layers
                ...
                if hasattr(layer.cell, 'weights_kernel'):
                    input_dim, _ = layer.cell.weights_kernel.shape
                    layer_params += np.size(layer.cell.weights_kernel)
                if hasattr(layer.cell, 'weights_recurrent'):
                    _, output_dim = layer.cell.weights_recurrent.shape
                    output_dim = output_dim // 4  # LSTM has 4 gates
            ...
            # Bidirectional layers
                ...
                # Forward LSTM
                if hasattr(layer.forward_layer.cell, 'weights_kernel'):
                    input_dim, _ = layer.forward_layer.cell.weights_kernel.shape
                    layer_params += np.size(layer.forward_layer.cell.weights_kernel)
                if hasattr(layer.forward_layer.cell, 'weights_recurrent'):
                    _, output_dim = layer.forward_layer.cell.weights_recurrent.shape
                    output_dim = output_dim // 4  # LSTM has 4 gates
